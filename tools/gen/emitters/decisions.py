...
from .effects import render_conditions, render_effects
...
def emit(ctx: BuildContext) -> None:
    ...
    for cat in categories:
        ...
        for d in cat.get("decisions", []) or []:
            ...
            db.add("complete_effect", render_effects(did, d["effects"], known_ideas, effects_used, where=SOURCE))
            ai = Block()
...
def _check_fields(vanilla: dict, ctx: BuildContext) -> None:
    if ctx.vanilla is None:
        return
